...
export async function deployProxy<T extends ethers.BaseContract>(
    ...
    if (hre.network.zksync) {
        ...
        const contractFactory = new zk.ContractFactory<any[], zk.Contract>(
            artifact.abi,
            artifact.bytecode,
            owner
        );
        ...
        const {impl} = await deployProxyImpl(hre, contractFactory as any, {
            ...
            factoryDeps: await extractFactoryDeps(hre, artifact),
        });
        ...
        const res = contractFactory.attach(await proxy.getAddress()) as any;
        res.deploymentTransaction = () => deploymentTransaction;
        return res;
    } else {
...
export async function upgradeProxy<T extends ethers.BaseContract>(
    ...
    if (hre.network.zksync) {
        const artifact = await hre.deployer.loadArtifact(newImpl);
        const contractFactory = new zk.ContractFactory<any[], zk.Contract>(
            artifact.abi,
            artifact.bytecode,
            admin
